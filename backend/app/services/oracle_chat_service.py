...
SYSTEM_PROMPT = (
    ...
    "- PENTING soal satuan nilai uang dari tools — jangan tertukar, daftar ini sudah "
    "diverifikasi langsung ke isi tabel:\n"
    "  RUPIAH PENUH (raw IDR), JANGAN dianggap 'juta': get_purchase_order_detail dan "
    "get_sales_order_detail (amount_idr/unit_price_idr), get_purchasing_performance (po_value), "
    "get_financial_summary (net_profit_*, cf_*).\n"
    "  JUTA IDR: get_sales_performance (bp_amount/actual_amount), get_cogs_performance "
    "(cogs_*/sales_amount/opex/ebit), get_budget_vs_actual (bp_amount/actual_amount).\n"
    "  get_ar_ap_summary dan get_inventory_summary mencampur keduanya dalam satu baris "
    "(mis. dso_sales dalam juta, dso_ar_avg dalam rupiah penuh) — sebutkan angkanya apa "
    "adanya beserta nama kolomnya, jangan menyimpulkan satuan, dan jangan menjumlahkan "
    "kolom-kolom itu satu sama lain.\n"
    "  Kalau ragu tool mana yang dipakai, cek nama tool di hasil sebelumnya.\n"
    "- Kalau angka hasil tool terasa tidak masuk akal setelah konversi (mis. total pembelian "
    "setahun jadi ratusan triliun), PERCAYA daftar satuan di atas, bukan firasat. Daftar itu "
    "sudah dicocokkan dengan isi tabel. Jangan menuliskan catatan keraguan soal satuan di "
    "jawaban — itu membuat user ragu pada angka yang sebenarnya benar. Kalau memang ada yang "
    "janggal, sebutkan singkat apa yang janggal, bukan berteori tentang satuan.\n"
